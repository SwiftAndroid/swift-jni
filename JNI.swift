import Glibc // on my system the Glibc modulemap also imports jni.h, should be a separate JNI module though
...
public class JNI {
    ...
    // We init the jni global ourselves in JNI_OnLoad
    private init?(jvm: UnsafeMutablePointer<JavaVM>) {
...
public extension JNI {
    public func RegisterNatives(jClass: jclass, methods: [JNINativeMethod]) -> Bool {
		...
		if env.ExceptionCheck(_env) != 0 {
		    env.ExceptionDescribe(_env)
		    env.ExceptionClear(_env)
		}
